...
1. [MInDS-14](https://hf-mirror.com/datasets/PolyAI/minds14) データセットで [Wav2Vec2](https://hf-mirror.com/facebook/wav2vec2-base) を微調整して話者の意図を分類します。
2. 微調整したモデルを推論に使用します。
...
2. オーディオ ファイルのサンプリング レートが、モデルが事前トレーニングされたオーディオ データのサンプリング レートと一致するかどうかを確認します。この情報は、Wav2Vec2 [モデル カード](https://hf-mirror.com/facebook/wav2vec2-base) で見つけることができます。
